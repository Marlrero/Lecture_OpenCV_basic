...
def preprocessing(no):  # 검출 전처리
    image = cv2.imread(f'face/{no}.jpg', cv2.IMREAD_COLOR)
    if image is None: return None, None
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)  # 명암도 영상 변환
    gray = cv2.equalizeHist(gray)  # 히스토그램 평활화
    return image, gray

face_cascade = cv2.CascadeClassifier("haarcascades/haarcascade_frontalface_alt2.xml")  # 정면 검출기
eye_cascade = cv2.CascadeClassifier("haarcascades/haarcascade_eye.xml")  # 눈 검출기
image, gray = preprocessing(0)  # 전처리
